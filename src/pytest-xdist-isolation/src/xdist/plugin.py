...
def pytest_xdist_make_scheduler(config: pytest.Config, log):
    if config.getoption("dist") == "isolation":
        from xdist.scheduler.isolation import IsolatedScheduler

        return IsolatedScheduler(config, log)
    return None
...
def pytest_cmdline_main(config: pytest.Config) -> None:
    ...
    if config.option.dist == "isolation":
        tx_specs = list(config.option.tx)
        if not tx_specs:
            raise pytest.UsageError(
                "--dist=isolation requires specifying workers via -n or --tx"
            )
        if len(tx_specs) < 2:
            tx_specs.append(tx_specs[-1])
        config.option.tx = tx_specs
